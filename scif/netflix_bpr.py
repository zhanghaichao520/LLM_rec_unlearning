...
class SCIF_Unlearner:
    def __init__(self, model, data_generator, device,
        ...
        """
        model: 已训练好的 Recbole BPR 模型，其 user_embedding 和 item_embedding
...
# ------------------------------------------------------------
# 4. 主流程：训练 Recbole BPR → SCIF Unlearning → 评估
# ------------------------------------------------------------
def train_and_unlearn(args):
    ...
    # 4.2. 用 Recbole 训练 BPR（全量训练集：train_normal + train_random）
    print("\n=== Training BPR on full dataset ===")
    config_full = Config(
        model=args.model,
    ...
    bpr_model = get_model(config_full["model"])(config_full, train_data._dataset).to(device)
    trainer_full = get_trainer(config_full["MODEL_TYPE"], config_full["model"])(config_full, bpr_model)

    trainer_full.fit(train_data, verbose=True)
    save_full = "./saved/bpr_full.pth"
    os.makedirs(os.path.dirname(save_full), exist_ok=True)
    torch.save(bpr_model.state_dict(), save_full)

    # 4.3. 预遗忘评估（Full BPR）
    print("\n=== Pre-Unlearn Evaluation (Full BPR) ===")
    ...
    save_unlearned = "./saved/bpr_unlearned.pth"
    unlearner.run(save_unlearned)

    # 4.5. 后遗忘评估（BPR Unlearned）
    print("\n=== Post-Unlearn Evaluation (BPR Unlearned) ===")
    bpr_un = get_model(config_full["model"])(config_full, train_data._dataset).to(device)
    bpr_un.load_state_dict(torch.load(save_unlearned))
    trainer_un = get_trainer(config_full["MODEL_TYPE"], config_full["model"])(config_full, bpr_un)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model",   "-m", default="BPR", help="Recbole 模型名（BPR）")
