...
class QuickTrainer:
    def __init__(self, data_generator):
        ...
        # 定义保存路径
        self.save_path = './Weights/LightGCN/Quick_LightGCN_ml1m.pth'
        os.makedirs(os.path.dirname(self.save_path), exist_ok=True)
        print(f"模型将保存到: {self.save_path}")
        
    def train_quick(self, epochs=5):
        """快速训练几个epoch"""
        print(f"\n开始快速训练 {epochs} 轮...")
...
def main():
    ...
    try:
        ...
        model_path = trainer.train_quick(epochs=5)  # 快速训练5轮
        
        # 打印使用此模型进行IFRU测试的命令
        print("\n"+"="*80)
        print("使用此快速训练模型测试IFRU:")
        print(f"python run_ifru_from_pretrained.py --model_path={model_path} --embed_size=32")
        print("="*80)
