...
def parse_args():
    parser = argparse.ArgumentParser(description="Influence Function Recommendation Unlearning (IFRU) for BPR - ML-1M")
    
    # Model path
    parser.add_argument('--model_path', type=str, 
                        default='./Weights/BPR/Quick_BPR_ml1m.pth',
    ...
    parser.add_argument('--embed_size', type=int, default=64,
                        help='Embedding dimension (larger for ML-1M)')
    ...
    parser.add_argument('--batch_size', type=int, default=2048,
                        help='Batch size (larger for ML-1M)')
    ...
    parser.add_argument('--max_train_samples', type=int, default=100000,
                        help='Maximum training samples (larger for ML-1M)')
    
    return parser.parse_args()


class ML1MDataGenerator:
    """ML-1M data generator, compatible with Data_for_LightGCN interface"""
    ...
    def __init__(self, args, path):
        ...
        # Create labels - ratings>=4 are positive samples for ML-1M
        ...
        # ML-1M is larger, adjust subset size accordingly
    ...
    def split_data(self, data, test_ratio=0.2):
        ...
        for user_id in range(self.n_users):
            ...
            if len(user_data) >= 5:  # At least 5 interactions for ML-1M
...
class RecBoleDatasetAdapter:
    """Adapter class, adapt ML1MDataGenerator to RecBole expected format"""
...
def create_recbole_config(embed_size):
    ...
    config_dict = {
        ...
        'train_batch_size': 2048,  # Larger for ML-1M
        'eval_batch_size': 2048,   # Larger for ML-1M
        'learning_rate': 0.001,    # Adjusted for ML-1M
        'weight_decay': 1e-4,
...
def evaluate_model_leave_one_out(model, data_generator, topks=[5, 10, 20], num_neg=99, 
    ...
    """Improved Leave-One-Out evaluation - optimized for larger dataset"""
    model.eval()
    with torch.no_grad():
        ...
        # Sample users for ML-1M (too large otherwise)
...
def get_eval_result_bpr(data_generator, model, mask, test_on_remain=False, test_on_forget=False, forget_users_set=None):
    ...
    with torch.no_grad():
        ...
        # Sample test data (ML-1M is large)
...
class ReallySlowIFRUBPR(nn.Module):
    """Original really slow IFRU algorithm implementation - BPR version for ML-1M"""
    ...
    def compute_neighbor_influence_clip(self, data_generator, k_hop=1):
        ...
        # Use appropriate filtering criteria for ML-1M
    ...
    def compute_hessian_with_test(self, model=None, data_generator=None):
        """Original really slow IFRU algorithm - BPR version for ML-1M"""
        ...
        # Sample training data if needed (ML-1M is large)
        ...
        def compute_batch_gradients(u_params, i_params, data_batch, batch_size=500):  # Larger batch size for ML-1M
        ...
        for epoch in range(self.if_epoch):
            ...
            with torch.no_grad():
                # ML-1M requires smaller update scale due to larger dataset
...
def main():
    ...
    print("="*80)
    print("Influence Function Recommendation Unlearning (IFRU) for BPR - ML-1M")
    ...
    # Load data
    print("\nLoading data...")
    data_path = '/data/IFRU-main/Data/ml-1m'
    data_generator = ML1MDataGenerator(model_args, path=data_path)
    ...
    save_name = f"{save_dir}/ifru_bpr_ml1m_lr{args.if_lr}_damping{args.damping}.pth"
 ...
 # Use IFRU algorithm
    ...
    results_path = f'{result_dir}/ifru_bpr_ml1m_lr{args.if_lr}_damping{args.damping}_results.json'
