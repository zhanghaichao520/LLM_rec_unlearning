...
class QuickBPRTrainer:
    def __init__(self, data_generator):
        ...
        # 定义保存路径
        self.save_path = './Weights/BPR/Quick_BPR_ml1m.pth'
    ...
    def train_quick(self, epochs=1000):
        """训练BPR模型"""
        print(f"\n开始训练BPR模型 {epochs} 轮...")
        ...
        for epoch in range(epochs):
            ...
            # 使用更大的批次进行训练
            batch_size = min(8192, len(train_data))  # 增加批次大小
...
def main():
    ...
    print("="*80)
    print("训练BPR模型用于IFRU测试 (1000轮完整训练)")
    print("="*80)
    ...
    try:
        ...
        print(f"\n开始1000轮完整训练...")
        print(f"预计训练时间: ~{1000 * 0.1 / 60:.1f} 分钟")
        
        model_path = trainer.train_quick(epochs=1000)  # 完整训练1000轮
        
        # 打印使用此模型进行IFRU测试的命令
        print("\n"+"="*80)
        print("使用此训练好的模型测试IFRU:")
        print(f"python full_bpr_ifru_1m_fixed.py --model_path={model_path} --embed_size=32")
        print("="*80)
