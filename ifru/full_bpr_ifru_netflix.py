...
def parse_args():
    parser = argparse.ArgumentParser(description="Influence Function Recommendation Unlearning (IFRU) for BPR - Netflix")
    
    # Model path
    parser.add_argument('--model_path', type=str, 
                        default='./Weights/BPR/Quick_BPR_netflix.pth',
    ...
    parser.add_argument('--embed_size', type=int, default=32,
                        help='Embedding dimension for Netflix')
    ...
    parser.add_argument('--batch_size', type=int, default=2048,
                        help='Batch size (larger for Netflix)')
...
class NetflixDataGenerator:
    """Netflix data generator, compatible with ML-1M interface"""
    ...
    def __init__(self, args, path):
        ...
        try:
            # First try to read the first line to detect the format
            with open(inter_file, 'r') as f:
                first_line = f.readline().strip()
            
            print(f"First line content: {first_line}")
            
            # Check if it's RecBole format with field descriptions
            if ':' in first_line and ('user_id' in first_line or 'item_id' in first_line):
                # Parse field names
                field_specs = first_line.split()
                field_names = []
                for spec in field_specs:
                    if ':' in spec:
                        ...
                        field_names.append(spec)
                
                print(f"Parsed field names: {field_names}")
                
                # Read data, skipping first line
                data = pd.read_csv(inter_file, sep='\t', names=field_names, skiprows=1)
            else:
                # Standard format
            ...
            if 'user' not in data.columns or 'item' not in data.columns:
                raise ValueError(f"Could not find necessary user or item columns, available columns: {list(data.columns)}")
            ...
            # Create labels - ratings>=4 are positive samples
            if 'rating' in data.columns:
                data['label'] = (data['rating'] >= 4.0).astype(float)
            else:
                data['label'] = 1.0
            
            # Data sampling - Netflix dataset is extremely large
            if len(data) > 500000:  # If more than 500K records
                print(f"Dataset too large ({len(data)} records), sampling to 500K...")
                data = data.sample(n=500000, random_state=42).reset_index(drop=True)
            ...
            subset_size = min(len(self.train)//10, 5000)  # 10% or max 5000
...
class RecBoleDatasetAdapter:
    """Adapter class, adapt NetflixDataGenerator to RecBole expected format"""
...
def create_recbole_config(embed_size):
    ...
    config_dict = {
        ...
        'train_batch_size': 2048,  # Larger for Netflix
        'eval_batch_size': 2048,   # Larger for Netflix
        'learning_rate': 0.001,    # Adjusted for Netflix
        'weight_decay': 1e-4,
...
def evaluate_model_leave_one_out(model, data_generator, topks=[5, 10, 20], num_neg=99, 
    ...
    """Improved Leave-One-Out evaluation - optimized for Netflix dataset"""
    model.eval()
    with torch.no_grad():
...
def get_eval_result_bpr(data_generator, model, mask, test_on_remain=False, test_on_forget=False, forget_users_set=None):
    ...
    with torch.no_grad():
        ...
        # Sample test data (Netflix is large)
        if len(test_data) > 10000:
            indices = np.random.choice(len(test_data), 10000, replace=False)
...
class ReallySlowIFRUBPR(nn.Module):
    """Original really slow IFRU algorithm implementation - BPR version for Netflix"""
    
    def __init__(self, save_name, if_epoch=2000, if_lr=5e-5, k_hop=1, init_range=1e-8, 
                 damping=1e-3, scale=1.0, cg_iterations=700, cg_min_iter=40):
    ...
    def compute_neighbor_influence_clip(self, data_generator, k_hop=1):
        ...
        # Use appropriate filtering criteria for Netflix
    ...
    def compute_hessian_with_test(self, model=None, data_generator=None):
        """Original really slow IFRU algorithm - BPR version for Netflix"""
        ...
        # Sample training data if needed (Netflix is very large)
        if len(train_data) > self.args.max_train_samples:
            ...
            print(f"Sampled {len(train_data)} training interactions due to large dataset")
        ...
        for epoch in range(self.if_epoch):
            ...
            with torch.no_grad():
                # Netflix requires smaller update scale due to larger dataset
                update_scale = step_size / max(data_generator.n_train, 5000)
...
def main():
    ...
    print("="*80)
    print("Influence Function Recommendation Unlearning (IFRU) for BPR - Netflix")
    ...
    data_generator = NetflixDataGenerator(model_args, path=data_path)
    ...
    save_name = f"{save_dir}/ifru_bpr_netflix_lr{args.if_lr}_damping{args.damping}.pth"
    ...
    print("\nAfter unlearning:")# filepath: /data/IFRU-main/full_bpr_ifru_netflix.py
    for k_idx, k in enumerate(args.topks):
    ...
    results = {
        ...
        'auc': {
            ...
            'test_auc_or': float(test_auc_or),
            'test_auc_and': float(test_auc_and),
            'remain_auc': float(remain_test_auc),
            'remain_auc_or': float(remain_test_auc_or),
            'remain_auc_and': float(remain_test_auc_and),
            'forget_auc': float(forget_test_auc),
            'forget_auc_or': float(forget_test_auc_or),
            'forget_auc_and': float(forget_test_auc_and)
        },
        'metrics': {
            'before': {k: float(before_metrics['hit_ratio'][i]) for i, k in enumerate(args.topks)},
            'after': {k: float(after_metrics['hit_ratio'][i]) for i, k in enumerate(args.topks)}
        },
        'ndcg': {
            'before': {k: float(before_metrics['ndcg'][i]) for i, k in enumerate(args.topks)},
            'after': {k: float(after_metrics['ndcg'][i]) for i, k in enumerate(args.topks)}
        },
        'remain_metrics': {
            'before': {k: float(before_remain_metrics['hit_ratio'][i]) for i, k in enumerate(args.topks)},
            'after': {k: float(after_remain_metrics['hit_ratio'][i]) for i, k in enumerate(args.topks)}
        },
        'remain_ndcg': {
            'before': {k: float(before_remain_metrics['ndcg'][i]) for i, k in enumerate(args.topks)},
            'after': {k: float(after_remain_metrics['ndcg'][i]) for i, k in enumerate(args.topks)}
        },
        'forget_metrics': {
            'before': {k: float(before_forget_metrics['hit_ratio'][i]) for i, k in enumerate(args.topks)},
            'after': {k: float(after_forget_metrics['hit_ratio'][i]) for i, k in enumerate(args.topks)}
        },
        'forget_ndcg': {
            'before': {k: float(before_forget_metrics['ndcg'][i]) for i, k in enumerate(args.topks)},
            'after': {k: float(after_forget_metrics['ndcg'][i]) for i, k in enumerate(args.topks)}
        },
    ...
    results_path = f'{results_dir}/ifru_bpr_netflix_lr{args.if_lr}_damping{args.damping}_khop{args.k_hop}_results.json'
    ...
    print(f"Hessian computation time: {ifru.time_records['hessian_computation']:.2f}s")
    print(f"Influence function time: {ifru.time_records['influence_function']:.2f}s")
    print(f"Optimization time: {ifru.time_records['optimization']:.2f}s")
    print(f"Evaluation time: {ifru.time_records['evaluation']:.2f}s")
