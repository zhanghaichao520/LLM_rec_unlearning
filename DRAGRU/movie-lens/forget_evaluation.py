...
def calculate_hr(ground_truth, predictions):
    ...
    for user, items in predictions.items():
        ...
        if user in ground_truth:
            for item in items:
                if int(item) in ground_truth[user]:
                    hits += 1
                    break  # 一个用户命中一次即可
...
def ndcg_at_k(ground_truth, predictions, k):
    ...
    def dcg_at_k(relevant_items, recommended_items, k):
        ...
        for i in range(min(k, len(recommended_items))):
            ...
            relevance = 1 if int(item_id) in relevant_items else 0
...
file_path = 'ml-100k_LightGCN_prompt_top50_SelectionStrategy.RANDOM_forget.json_Meta-Llama-3-8B-Instruct_result_llm.csv'
